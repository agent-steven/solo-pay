  ...
  properties: {
    ...
    data: {
      ...
      properties: {
        ...
        expiresAt: { type: 'string', format: 'date-time', description: 'Payment expiry time' },
        currency: { type: 'string', description: 'Fiat currency code' },
        fiatAmount: { type: 'number', description: 'Fiat amount' },
        tokenPrice: { type: 'number', description: 'Token price at creation' },
        txHash: { type: 'string', description: 'Transaction hash from DB' },
      },
    },
  },
} as const;
