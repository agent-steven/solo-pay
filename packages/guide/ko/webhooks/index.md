...
Callback URL(`successUrl`/`failUrl`)은 사용자의 브라우저를 리다이렉트할 뿐입니다 — 사용자가 브라우저를 닫으면 서버는 결제 성공 여부를 알 수 없습니다. **Webhook은 결제 상태를 안정적으로 수신하는 유일한 방법입니다.** DB 업데이트, 주문 완료, 상품 발송 등의 처리는 반드시 Webhook 핸들러에서 수행하세요.
